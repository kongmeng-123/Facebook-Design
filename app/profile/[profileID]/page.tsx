...
const Profile = () => {
    ...
    const taps: string[] = ["All", "About", "Reals", "Photos", "Friends"];
    ...
    return (
        <div>
            <div className="h-auto w-full flex flex-col items-center">
                <div className="h-[25vh] sm:h-[40vh] md:h-[60vh] w-full flex justify-center relative">
                    ...
                    <div className="h-full w-full max-w-280 bg-blue-400 relative overflow-hidden rounded-b-xl ">
                        <Image src={wallpaper1} alt="background profile" fill className="object-cover" />
                    </div>
                </div>
                <div className="h-auto w-full max-w-260 ">
                    <div className="h-40 w-full  my-10 flex">
                        ...
                        <div className="h-full w-full bg-white">
                            <div className="h-10 w-full flex space-x-2 justify-end">
                                <button className="h-full px-2 flex items-center bg-second/20 hover:bg-second/30 rounded-lg space-x-2" >
                                    <FaFacebookMessenger />
                                    <span>Message</span>
                                </button>
                                <button className="h-full px-2 flex items-center bg-primary text-white hover:bg-primary/90 rounded-lg space-x-2" onClick={() => isFollow ? setIsFollow(false) : setIsFollow(true)} >
                                    <ImBoxAdd />
                                    <span>{isFollow ? 'following' : 'follow' }</span>
                                </button>
                                <button className="h-full px-2 flex items-center bg-second/20 hover:bg-second/30 rounded-lg space-x-2" onClick={() => addFriend ? setAddFriend(false):setAddFriend(true)}>
                                ...
                                <button className="h-full px-4 flex items-center bg-second/20 hover:bg-second/30 rounded-lg text-2xl" onClick={() => (seeMorePeople ? setSeeMorePeople(false) : setSeeMorePeople(true))}>
                            ...
                            <ul className="h-[85%] flex space-x-2 overflow-x-scroll no-scrollbar">
                                {data.map((item) => (
                                    <li className="flex-shrink-0 h-full w-32 sm:w-36 bg-white  rounded-2xl overflow-hidden relative shadow-2xl group" key={item.item.id}>
                                        <div>
                                            <div className="w-full aspect-square sm:h-40 bg-blue-300 relative">
                    ...
                    <div className="h-13 w-full flex items-center justify-between">
                        <ul className="h-11 flex space-x-2">
                            {taps.map((tap) => (
                                <li className="h-full px-4 flex flex-col justify-center items-center rounded-lg hover:bg-second/20 font-medium text-second relative" key={tap} onClick={() => setCurrent(tap)}>
                                    <Link href={"#"}>{tap}</Link>
                                    {current == tap && <span className={`w-full h-1 bg-primary absolute bottom-[-4px] `}></span>}
                                </li>
                            ))}
                            <li className="h-full px-4 flex flex-col justify-center items-center rounded-lg hover:bg-second/20 font-medium text-second relative">
                        ...
                        <div className="h-8 w-12 bg-second/40 rounded-lg flex justify-center items-center text-lg ">
                            <BsThreeDots />
                        </div>
                    </div>
                </div>
